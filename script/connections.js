...
showconnects:function(elem,pin){
  var connector=webtronics.circuit.getwithselector('#information > .schematic_connector ,#information > .webtronics_namewire_connector')
...
removeconnects:function(){
  
  var connector=webtronics.circuit.getwithselector('#information > .schematic_connector,#information > .webtronics_namewire_connector,#information > .webtronics_schematic_wire_eventline ')
  for(var i=0;i<connector.length;i++)connector[i].parentNode.removeChild(connector[i]);
},

  
addconnects:function(){
 webtronics.circuit.changeobserver.disconnect();

  this.removeconnects();
  var parts=this.getwtxdata(webtronics.circuit.getwithselector('#webtronics_drawing > g'));
  this.connectnamewires(parts);
  for(var i=0 ;i<parts.length;i++){
    if(parts[i].analogpins!=undefined)for(var j=0;j<parts[i].analogpins.length;j++){
	var circle=webtronics.circuit.createdot('red',parts[i].analogpins[j].x,parts[i].analogpins[j].y,4);
	circle.setAttributeNS(null, 'fill-opacity', .35);
	circle.setAttribute('class',"schematic_connector");
	circle.setAttribute("pointer-events","all");
	circle.setAttribute('visibility','hidden');
	webtronics.circuit.info.appendChild(circle);
	...
	Event.observe(circle,"mouseout",function(){
	  if(!webtronics.circuit.connections){
	    var data = $A(arguments);
	    data.shift();							
	...
	Event.observe(circle,"mousedown",function(){
	  var data = $A(arguments);
	  data.shift();							
	  if(webtronics.circuit.mode=='select'){
	    webtronics.setMode('line','Wire');
	    var svg = webtronics.circuit.createline('blue',2, data[0], data[1], data[0], data[1]);
	    svg.id = 'templine1';
	    svg.setAttributeNS(null,'stroke-dasharray','3,2');
	    webtronics.circuit.info.appendChild(svg);
	  }
	  else{
	    webtronics.circuit.wiresegment();
	    webtronics.circuit.wiresegment();
	    
	    webtronics.circuit.remove(webtronics.circuit.getwithselector("#templine1")[0]);
	...
	var rect=webtronics.circuit.createrect('red',100,parts[i].digitalpins[j].x-3,parts[i].digitalpins[j].y-3,6,6);
	rect.setAttributeNS(null, 'fill-opacity', .35);
	rect.setAttribute('class',"schematic_connector");
	rect.setAttribute("pointer-events","all");
	rect.setAttribute("visibility","hidden");
	webtronics.circuit.info.appendChild(rect);
  ...
  var lines=webtronics.circuit.getwithselector("#webtronics_drawing > line");
  for(var i=0;i<lines.length;i++){
//clean up lines while i'm at it    
    if(lines[i].getAttributeNS(null,'x1')==lines[i].getAttributeNS(null,'x2') && lines[i].getAttributeNS(null,'y1')==lines[i].getAttributeNS(null,'y2'))
      webtronics.circuit.remove(lines[i]);

    else webtronics.circuit.wireevents(lines[i]);

    
  }
  this.hideconnects();  
  webtronics.circuit.changeobserver.observe(webtronics.circuit.drawing, { attributes: true, childList: true, characterData: true ,subtree:true});
